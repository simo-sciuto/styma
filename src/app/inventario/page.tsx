...
function priceNote(item: ItemRow): string | null {
  ...
  if (item.purchase_price !== null)
    return `Pagato ${formatEur(item.purchase_price)}`;
  if (item.asking_price !== null)
    return `Chiedevano ${formatEur(item.asking_price)}`;
  return null;
}
      ...
      {result.status === "not_configured" ? (
        ...
        <Card className="mt-6 border-danger/40 bg-danger-soft">
          <p className="text-sm text-danger">
            Non riusciamo a raggiungere l’inventario. I tuoi oggetti sono al
        ...
        <Card className="mt-6">
          ...
          {result.archived > 0 ? (
            ...
            <p className="text-sm text-muted">
              Ancora niente. Analizza un oggetto e lo ritrovi qui, con la stima
                  ...
                  >
                    ...
                    <div className="flex min-w-0 flex-1 flex-col justify-center sm:block sm:p-4">
                      ...
                      <p className="mt-0.5 truncate text-sm text-muted">
                        {[item.brand, item.estimated_period]
                          .filter(Boolean)
                          .join(" · ") || item.category}
                      </p>
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        ...
                        <Pill
                          tone={item.status === "sold" ? "accent" : "neutral"}
                        >
                          {ITEM_STATUS_LABELS[item.status]}
                        </Pill>
                        {item.archived_at !== null ? (
                          <Pill>Archiviato</Pill>
                        ) : null}
